...
function showPage(page) {
    ...
    if (page === 'home') {
        ...
        content.innerHTML = `
            ...
            <form onsubmit="createAuction(event)">
                ...
                <div class="form-group">
                    ...
                    <!-- Mobile Guide -->
                    <div style="background:linear-gradient(135deg,rgba(108,92,231,0.1),rgba(255,51,102,0.1));padding:15px;border-radius:12px;margin-bottom:15px;border:2px solid #6C5CE7;">
                        <p style="font-weight:800;font-size:1.1rem;margin-bottom:10px;">📱 Kako odabrati 10 slika odjednom:</p>
                        <div style="background:#0F1629;padding:12px;border-radius:8px;margin-top:10px;">
                            <p style="margin:5px 0;"><strong style="color:#00E396;">1.</strong> Klikni upload zonu 👇</p>
                            <p style="margin:5px 0;"><strong style="color:#00E396;">2.</strong> <strong>DRŽI PRST</strong> na prvoj slici (2s)</p>
                            <p style="margin:5px 0;"><strong style="color:#00E396;">3.</strong> Pojave se ✅ checkboxovi</p>
                            <p style="margin:5px 0;"><strong style="color:#00E396;">4.</strong> Klikaj na 9 više slika</p>
                            <p style="margin:5px 0;"><strong style="color:#00E396;">5.</strong> Klikni "Done" / "OK"</p>
                        </div>
                    </div>
                    
                    <div class="upload-zone" onclick="document.getElementById('fileInput').click()">
                        <div class="upload-icon">📸</div>
                        <p style="font-weight:700;font-size:1.2rem;color:#6C5CE7;">ODABERI SLIKE</p>
                        <p style="font-size:0.9rem;color:#888;margin-top:8px;">Klikni ovdje da otvoriš galeriju</p>
                    </div>
                    <input type="file" id="fileInput" multiple accept="image/*" style="display:none" onchange="handleFiles(this.files)">
                    <div class="preview-grid" id="previewGrid"></div>
                    <div id="imageCounter" style="text-align:center;margin:15px 0;font-weight:700;font-size:1.1rem;">0/10 slika</div>
                </div>
        ...
        content.innerHTML = `
            <h1 class="page-title">Favoriti</h1>
        ...
        return `
            <div class="auction-card" onclick="openAuctionDetail('${auction.id}')">
                <div class="auction-image-wrapper">
                    <img src="${auction.slike[0]}" alt="${auction.naslov}" class="auction-image" loading="lazy">
...
function loadAuctions() {
    ...
    db.collection('aukcije')
        ...
        .onSnapshot(snapshot => {
            ...
            const grid = document.getElementById('auctionsGrid');
            if (grid) {
                grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:40px;color:#888;">Greška pri učitavanju oglasa</div>';
            }
        });
}
...
function openAuctionDetail(auctionId) {
    ...
    container.innerHTML = `
        <div class="modal active" onclick="if(event.target===this) closeModal()">
            <div class="modal-content auction-detail-modal" onclick="event.stopPropagation()">
                <button class="modal-close" onclick="closeModal()">×</button>
                
                <!-- IMAGE GALLERY -->
                <div class="auction-detail-gallery">
                    <img src="${auction.slike[0]}" id="mainImage" class="auction-detail-main-image">
                ...
                <div class="action-buttons">
                    <button class="btn-chat" onclick="openChat('${auction.userId}', '${auction.userName}')">
                        💬 Chat
                    </button>
                    <button class="btn-rate" onclick="openRating('${auction.userId}', '${auction.userName}')">
                        ⭐ Ocijeni
                    </button>
                </div>
            </div>
